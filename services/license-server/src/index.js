...
export default {
  async fetch(request, env) {
    ...
    // Only the three public app endpoints need browser CORS. The admin panel, Checkout and Stripe
    // all use this origin (or server-to-server requests), so advertising them cross-origin would
    // widen the attack surface without enabling a product capability.
...
async function redeemKey(deviceId, keyCode, env, now) {
  ...
  const results = await env.DB.batch([
    ...
    env.DB.prepare(
      `UPDATE devices
       SET status = 'ACTIVE', purchased_at = ?, expires_at = ?, note = ?, updated_at = ?
...
async function verifyRegistrationProof(body) {
  ...
  return { ok: true, deviceId, nonce, publicKey };
}
...
async function verifyDeviceProof(publicKeyBytes, proof, action, deviceId, nonce) {
  try {
    ...
    return await crypto.subtle.verify(
      ...
      key,
      proof,
      new TextEncoder().encode(canonicalDeviceProof(action, deviceId, nonce)),
    );
  } catch {
    return false;
  }
