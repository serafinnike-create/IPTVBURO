...
class QrCodeTest {
    ...
    @Test
    fun `versions seven through ten contain both protected version fields`() {
        val matrix = assertNotNull(QrCode.encode("A".repeat(180)))
        ...
        val expected = (version shl 12) or remainder
        for (index in 0 until 18) {
            val dark = (expected ushr index) and 1 == 1
            assertEquals(dark, matrix[matrix.size - 11 + index % 3, index / 3])
            assertEquals(dark, matrix[index / 3, matrix.size - 11 + index % 3])
        }
    }
